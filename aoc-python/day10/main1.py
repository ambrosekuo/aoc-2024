...
def get_trailhead_ends(arr3d, x, y):
    ...
    total_trailhead_ends = set()
    for [next_x, next_y] in get_next_possible_paths(arr3d, x,y):
        total_trailhead_ends |= get_trailhead_ends(arr3d, next_x, next_y)
    
    return total_trailhead_ends
...
def get_trails(arr3d):
    ...
    for y, line in enumerate(arr3d):
        for x, val in enumerate(line):
            if val == 0:
                total_trailhead_scores += len(get_trailhead_ends(arr3d, x, y))
