...
def get_cheats_count_old(game_area, path, path_map,  min_cheats = 100):
    cheat_count = 0
    cheats = []
    for [x, y] in path:
...
def is_reachable(current, next, cheat_step_limit):
    [x,y] = current
    [x2,y2] = next
    steps_taken = (abs(x2-x) + abs(y2-y))
    return steps_taken <= cheat_step_limit

def get_cheats_count(path, path_map, min_cheats = 100):
    cheats_count = 0
    cheats = []
    for [x,y] in path:
        path_index = path_map[key([x,y])]
        min_higher_path = min_cheats + path_index 
        if min_higher_path > len(path):
            break
        for [cx,cy] in path[min_higher_path:]:
            cheat_step_limit = (path_map[key([cx,cy])] - path_map[key([x,y])]) - min_cheats
            if is_reachable([x,y], [cx,cy], min(cheat_step_limit, 20)):
                cheats.append([[x,y], [cx,cy]])
                cheats_count +=1
    return cheats_count
...
print(get_cheats_count(path, path_map))